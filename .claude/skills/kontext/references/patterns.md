...
## Pattern 2: User Authorization Flow (Public Client)

When your skill needs access to external services, use the public client pattern — `clientId` + PKCE, no client secret. This is the standard approach for CLI tools, browser apps, and AI agents.

```typescript
import { KontextMcp } from '@kontext-dev/js-sdk';
import { toKontextTools } from '@kontext-dev/js-sdk/ai';

// Public client: no clientSecret needed — PKCE handles security
...
const { tools } = await toKontextTools(kontext);
